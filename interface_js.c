...
int smart_details(char *file)
{
	...
	char *disk = "sda";
	char command[48] = {0};
	char modelfam[32] = {0}, devmodel[32] = {0}, sernumber[32] = {0}, fwversion[32] = {0}, usercap[32] = {0};
...
//DONE RAID5
		...
		sprintf(command,"sh /usr/local/system/md_max",junk);
		get_data(command,md);
		mdnum = atoi(md);
...
*/
	...
	for (partition = 1; partition <=15; partition++) {
		sprintf(command, "mdadm --zero-superblock /dev/%s%d &>/dev/null ; parted -s /dev/%s rm %d &>/dev/null ", diskname, partition, diskname, partition);
		get_data(command,output);
	}
	sprintf(command, "parted -s /dev/%s mkpart SYS 21.0KB 1MB 2&>/dev/null", diskname);
	get_data(command,output);
	sprintf(command, "parted -s /dev/%s mkpart SYSRFS1 1MB 1GB 2&>/dev/null ", diskname);
	get_data(command,output);
	sprintf(command, "parted -s /dev/%s mkpart SWAP 1GB 2GB 2&>/dev/null ", diskname);
	get_data(command,output);
	sprintf(command, "parted -s /dev/%s mkpart BACKUP 2GB 3GB 2&>/dev/null ", diskname);
	get_data(command,output);

	sprintf(command, "cat /sys/class/net/p5p1/address | sed 's/://g'");
	get_data(command,mac);
	mac[12] = '\0';
   	sprintf(command, "echo %s | dd of=/dev/%s1 seek=512 bs=1", mac, diskname);
	get_data(command,output);
}
